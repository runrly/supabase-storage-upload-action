...
import { parseActionInputs } from "./input.js";
...
export async function run(): Promise<void> {
	...
	try {
		const inputs = parseActionInputs({
			...
			supabaseKey: core.getInput("supabase-key"),
			workspace: process.env.GITHUB_WORKSPACE ?? process.cwd(),
		});

		const config = parseConfig(inputs.config);
		const plan = await createUploadPlan(config, {
			workspace: inputs.workspace,
		});

		core.setOutput("matched-count", String(plan.entries.length));
		await writeSummary(plan.entries, inputs.dryRun);
		...
		core.setSecret(inputs.supabaseKey);

		const result = await executePlan(
			plan,
			createSupabaseUploader({
				supabaseUrl: inputs.supabaseUrl,
				supabaseKey: inputs.supabaseKey,
			}),
		);
